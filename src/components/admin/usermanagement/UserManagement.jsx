...
export default function UserManagement({ user }) {
    ...
    const handleOpenModal = async (item = null) => {
        ...
        if (item) {
            ...
            setFormData({
                ...
                parentPhone: full.parentEmail || '',   // parentEmail stores parent phone
    ...
    const validateForm = () => {
        ...
        const { name, email, school, rollNo, section, parentName,
            address, bloodGroup, dateOfBirth, password, phoneNumber, parentPhone } = formData
    ...
    const handleSaveUser = async () => {
        ...
        try {
            ...
            showSuccess(`${activeTab === 'schools' ? 'School' : 'User'} saved successfully!`)
            // Reload only the current level's data
    ...
    return (
        <div className="relative">
            ...
            {!loading && (
                <>
                    {/* ── LEVEL 1: School cards (student tab, no school selected) ── */}
                    ...
                    {/* ── LEVEL 2: Class cards (school selected, no class yet) ── */}
                    {activeTab === 'student' && selectedSchool && !selectedClass && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            {classesData
                                .sort((a, b) => {
                                    const na = parseInt(a.className?.match(/\d+/)?.[0] || 0)
                                    const nb = parseInt(b.className?.match(/\d+/)?.[0] || 0)
                                    return na - nb
                                })
                                .map(cls => (
                                    <div
                    ...
                    {/* ── LEVEL 3: Student table (school + class selected) ── */}
                    {activeTab === 'student' && selectedSchool && selectedClass && (
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {filteredStudents.map(u => {
                                        // Full data already available — no extra API call needed
                                        ...
                                        return (
                                            <React.Fragment key={u.id}>
                                                ...
                                                {/* Expanded row — data already in memory, zero extra API calls */}
                                                {expandedRow === u.id && (
