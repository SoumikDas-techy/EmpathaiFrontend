...
export default function UserManagement({ user }) {
    ...
    useEffect(() => {
        loadUsers()
        if (activeTab !== 'schools' && activeTab !== 'teacher') loadSchools()
    ...
    const handleOpenModal = async (userToEdit = null) => {
        ...
        if (userToEdit) {
            ...
            setFormData({
                ...
                parentPhone: fullUser.parentEmail || '',
                    ...
                    <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div className="flex items-center gap-3">
                            {selectedSchool && activeTab === 'student' && (
                                <button
                                    onClick={() => { if (selectedClass) setSelectedClass(null); else setSelectedSchool(null) }}
                                    ...
                                    <div key={sName} onClick={() => setSelectedSchool(sName)} className="bg-white p-6 rounded-xl border border-gray-200 hover:shadow-lg cursor-pointer">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <div className="bg-purple-50 text-purple-700 text-xs font-bold px-3 py-1 rounded-full">
                                                {schoolsMap[sName].count} Students
                                            </div>
                                        </div>
                                    ...
                                    .map(cName => (
                                        <div key={cName} onClick={() => setSelectedClass(cName)} className="bg-white p-6 rounded-xl border border-gray-200 hover:shadow-md cursor-pointer text-center">
                                            ...
                                            <div className="text-xs text-purple-800 bg-purple-100 px-2 py-1 rounded-full inline-block mt-1">
                                                {classesMap[cName].count} Students
                                            </div>
                                        </div>
                                    ))
                            })()}
